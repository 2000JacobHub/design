...
export function LibraryPanel({ open, layout, onReset, onDragStart, onDragEnd }: LibraryPanelProps) {
  return (
    <aside
      aria-hidden={!open}
      // Stays mounted so it can animate both directions. Width collapses to 0
      // (overflow-x-hidden clips the fixed-width inner content) while the
      // inner panel slides + fades out — combined feels like the panel
      // tucks itself away to the right.
      className={[
        "shrink-0 h-screen sticky top-0 overflow-y-auto overflow-x-hidden bg-cosmic-black/60 backdrop-blur",
        "transition-[width,opacity,border-color] duration-300 ease-out-expo",
        open
          ? "w-72 opacity-100 border-l border-soft-white/10"
          : "w-0 opacity-0 border-l border-transparent pointer-events-none",
      ].join(" ")}
    >
      <div
        className={[
          "w-72 p-4 transition-transform duration-300 ease-out-expo",
